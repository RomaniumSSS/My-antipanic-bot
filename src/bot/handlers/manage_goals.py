...
async def cmd_goals(message: Message, state: FSMContext) -> None:
    ...
    # Проверяем лимит целей (макс 10 активных/paused целей)
    active_goals = [g for g in goals if g.status in ("active", "paused")]
    can_add_goal = len(active_goals) < 10
...
async def on_create_goal(
    ...
    AICODE-NOTE: Проверяем лимит целей (макс 10 активных) перед созданием.
    """
    ...
    # Проверяем лимит целей (макс 10 активных/paused)
    ...
    if active_goals >= 10:
        await msg.edit_text(
            "⚠️ У тебя уже 10 активных целей (лимит).\n\n"
