...
- [x] Этап 2: Разделить слои (по сценариям, 3-5 дней) ✅
...
**Статус**: Завершен
**Дата**: 2025-12-13
**Коммит**: (будет создан) - docs(stage-2.6): mark Stage 2 as completed

**Что сделано**:
1. ✅ Проверка кода ruff linter:
   - Все файлы: `ruff check src/` → **All checks passed!**
   - Никаких ошибок линтера

2. ✅ Проверка Python синтаксиса:
   - Все use-cases, domain, handlers, storage → **компиляция успешна**
   - Никаких синтаксических ошибок

3. ✅ Проверка архитектуры слоев:
   - **Domain layer** (src/core/domain/): 5 файлов
   ...
   - **Use-case layer** (src/core/use_cases/): 5 файлов
     * assign_morning_steps.py - утренний сценарий
     * complete_daily_reflection.py - вечерний итог
     * complete_step.py - выполнение шага
     * resolve_stuck.py - помощь при застревании
     * skip_step.py - пропуск шага

   - **Storage layer** (src/storage/): 5 файлов
     * daily_log_repo.py - CRUD для DailyLog
     * goal_repo.py - CRUD для Goal/Stage
     * stage_repo.py - CRUD для Stage
     * step_repo.py - CRUD для Step
     * user_repo.py - CRUD для User

   - **Handler layer** (src/bot/handlers/): тонкие, только UI логика

4. ✅ Проверка экспортов use-cases:
   - assign_morning_steps_use_case - singleton экспорт ✓
   - complete_daily_reflection_use_case - singleton экспорт ✓
   - resolve_stuck_use_case - singleton экспорт ✓
   - complete_step_use_case, skip_step_use_case - локальные экземпляры в handler ✓
...
- ✅ Репозитории = тупые CRUD без бизнес-логики
- ✅ Домейн = чистые функции без БД и Telegram
- ✅ Use-cases = оркестрация репозиториев + домейн + AI
- ✅ Handlers = тонкие, только вызов use-case + UI
